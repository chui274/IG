...
//Tratamiento de los vertices
  ...
  for(int i = 0; i<num; i++){
    for(int j = 0; j<num_aux; j++){
      //Introducimos los vertices del perfil en el vector de vertices y Rotamos sobre Y
      vertice_aux.x = (aumento+perfil[j].x)*cos(2.0*M_PI*i/(1.0*num))-perfil[j].z*sin(2.0*M_PI*i/(1.0*num));
      vertice_aux.y = perfil[j].y;
      vertice_aux.z = (aumento+perfil[j].x)*sin(2.0*M_PI*i/(1.0*num))+perfil[j].z*cos(2.0*M_PI*i/(1.0*num));
      vertices.push_back(vertice_aux);
