...
_file_ply::_file_ply() {
  ...
  const char *Text_tokens[] = {"ply",        "format",   "ascii",
                               "element",    "vertex",   "face",
                               "end_header", "zzzzzzzz", ""};
...
int _file_ply::is_number(char *Aux_token) {
  ...
  for (unsigned int i = 0; i < strlen(Aux_token); i++) {
    if ((p1 = strchr((char *)"0123456789.+-eE", Aux_token[i])) == NULL)
      return (-1);
  }
  return (0);
}
...
int _file_ply::search_token(char *Aux_token) {
  ...
  while (!(Token_table[i].Text == "zzzzzzzz")) {
    ...
    if (Token_table[i].Text == Aux_token)
      break;
    i++;
  }
  // getchar();
